...
const udd = mkdtempSync(join(tmpdir(), "cdp-v14c-"));
const edge = spawn(EDGE, ["--headless=new","--disable-gpu",`--user-data-dir=${udd}`,"--remote-debugging-port=9564","--window-size=1600,900","about:blank"], { stdio: "ignore" });
const sleep=(ms)=>new Promise((r)=>setTimeout(r,ms));
async function getWs(){for(let i=0;i<30;i++){try{const l=await(await fetch("http://127.0.0.1:9564/json/list")).json();const p=l.find(t=>t.type==="page");if(p)return p.webSocketDebuggerUrl;}catch{}await sleep(300);}throw new Error("no cdp");}
...
await sleep(900);
const mid = await evalJs(`({active: document.getElementById('meteor-fx').classList.contains('active')})`);
console.log("t=900ms:", JSON.stringify(mid));
await sleep(1400);
await shot("fx-meteor-dark-hold2");
// 浅色
...
await mouse("mousePressed", 200, 650, { clickCount: 1 });
await sleep(1800);
await shot("fx-meteor-light-hold2");
await mouse("mouseReleased", 200, 650, { clickCount: 1 });
const end = await evalJs(`({active: document.getElementById('meteor-fx').classList.contains('active'), url: location.pathname})`);
