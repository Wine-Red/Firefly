...
pinned: false
category: 经验分享
title: Uptime Kuma 玻璃拟态、状态动效主题CSS分享
description: 分享一套自定义 CSS 代码，对 Uptime Kuma 状态监控面板进行美化。
image: ../../assets/images/picture/uptime-kuma-beautiful.png
...
最近给自己的 Uptime Kuma 状态页做了一套自定义 CSS。一开始只是想让状态页不要那么默认，后来去别人的博客搜寻了一些主题样式，然后便一发不可收拾...

本套主题把Uptime Kuma的状态页背景、卡片、状态点、Uptime 竖条都统一调整了一下，实现了以下一些效果：
...
在 Uptime Kuma 中应用自定义 CSS 非常简单：

1. 登录到你的 Uptime Kuma 管理后台。
2. 进入你所创建的**状态页面**。
3. 找到“**编辑状态页面**”按钮。
4. 在页面下方找到“**自定义 CSS**”输入框，将美化代码粘贴进去并保存即可。

## 完整CSS

直接复制下面的完整版代码到自定义 CSS 框中。该版本集成了背景图、毛玻璃卡片、圆角优化，并隐藏了底部信息。建议在 Uptime Kuma 的**深色模式**下使用以获得最佳效果。
...
需要注意的是，代码里的背景图地址是我自己的，如果要换成自己的背景，只需要替换这一行（73行，位于第一部分中）：
```css
background-image: url("https://winered-0v0.com/night.jpg");
```
...
![Uptime Kuma 效果展示](../../assets/images/picture/uptime-kuma-beautiful.png)
